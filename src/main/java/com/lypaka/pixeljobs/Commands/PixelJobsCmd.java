    ...
    public CommandResult execute(CommandSource src, CommandContext args) throws CommandException {
        ...
        if (job.equals("catch")) {
            if (!config.getNode("Catch", "Job 1").isVirtual()) {
                ...
                player.sendMessage(Text.of(TextColors.GOLD, "[", TextColors.DARK_RED, "PixelJobs", TextColors.GOLD, "] ", TextColors.WHITE, "No catch jobs exist right now! Try again later!"));
            }
        } else if (job.equals("kill")) {
            if (!config.getNode("Kill", "Job 1").isVirtual()) {
                pages.openKillJobsPage(player);
            } else {
                player.sendMessage(Text.of(TextColors.GOLD, "[", TextColors.DARK_RED, "PixelJobs", TextColors.GOLD, "] ", TextColors.WHITE, "No kill jobs exist right now! Try again later!"));
            }
        } else if (job.equals("evolve")) {
            if (!config.getNode("Evolve", "Job 1").isVirtual()) {
                pages.openEvolveJobsPage(player);
            } else {
                player.sendMessage(Text.of(TextColors.GOLD, "[", TextColors.DARK_RED, "PixelJobs", TextColors.GOLD, "] ", TextColors.WHITE, "No evolve jobs exist right now! Try again later!"));
            }
        }
